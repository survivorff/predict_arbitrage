...
2. **实时告警到 Telegram/Lark**（`TelegramChannel`/`LarkChannel`，已在迭代计划登记）— 机会几分钟即逝，告警是命脉。带净利率/规模/可靠性摘要 + 详情页深链。
3. **Kelly / bankroll 建议仓位** — 在风控规模上限内，按用户 bankroll 与净利率给出 **¼-Kelly** 建议仓位（竞品标配），并展示"投入→预期收益"。
